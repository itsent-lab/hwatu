...
describe('AI 난이도 기기 설정', () => {
  it('저장값이 없거나 잘못되면 보통을 사용한다', () => {
    ...
    expect(loadAiDifficulty(memoryStorage({ 'nsj-hwatu-ai-difficulty-v1': 'legend' }))).toBe('normal');
  });
  ...
  it('점당 100냥·1천냥·2천냥·5천냥·1만냥만 기억한다', () => {
    ...
    expect(loadPointValue(memoryStorage({ 'nsj-hwatu-point-value-v1': '500' }))).toBe(100);
  });
  ...
  it('고스톱 점당 금액은 맞고와 다른 설정으로 기억한다', () => {
    ...
    expect(loadGostopPointValue(memoryStorage({ 'nsj-hwatu-gostop-point-value-v1': '500' }))).toBe(100);
  });
