...
struct GameTableView: View {
    ...
    private func synchronize(_ session: GameSession) async {
        ...
        if deltas.values.allSatisfy({ $0 == 0 }) {
...
private struct GameBoard: View {
    ...
    private var automaticTaskID: String {
        let effectKey = session.mode == .matgo
            ...
            : ""
        return [
            String(autoPlay), String(hasStarted), String(isDealing), String(session.turnNumber),
            String(describing: session.phase), session.currentPlayer.rawValue,
            session.pendingMatch?.id.uuidString ?? "", session.pendingChongtong?.id ?? "",
            String(isGookjinChoicePresented), effectKey
        ].joined(separator: "|")
    }
...
private struct ArcadeRedButtonStyle: ButtonStyle {
...
private struct CompactGameButtonStyle: ButtonStyle {
    let active: Bool
...
private struct GameModalSecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
...
private struct ReserveExitButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
...
private struct ImmediateExitButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label.foregroundStyle(.white).padding(.horizontal, 15).frame(minHeight: 72)
...
private struct ExitCancelButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
