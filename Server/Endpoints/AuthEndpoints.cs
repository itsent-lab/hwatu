...
public static class AuthEndpoints
{
    public static IEndpointRouteBuilder MapAuthEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/status", async (UserRepository users) =>
            ApiResults.Ok(new { needsBootstrap = await users.CountAsync() == 0 }));
        ...
        SessionService sessions)
    {
        ...
        {
            return ApiResults.Error("INVALID_PASSWORD", "4자 이상의 동일한 비밀번호를 입력해 주세요.", 422);
        }
        ...
        try
        {
            var id = await users.CreateAsync(
                ...
                "admin");
            var user = await users.FindByIdAsync(id) ?? throw new InvalidOperationException("관리자 계정을 찾을 수 없습니다.");
