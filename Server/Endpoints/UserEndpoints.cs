...
public static class UserEndpoints
{
        ...
        IPasswordHasher<AppUser> passwordHasher)
    {
        ...
        {
            return ApiResults.Error("INVALID_MEMBER", "3자 이상의 한글·영문·숫자·밑줄 아이디, 표시 이름과 4자 이상의 비밀번호를 확인해 주세요.", 422);
        }
        ...
        IPasswordHasher<AppUser> passwordHasher)
    {
        ...
        if (!Validation.IsPassword(request.Password))
            return ApiResults.Error("INVALID_PASSWORD", "새 비밀번호는 4자 이상 입력해 주세요.", 422);
