...
function startBackgroundMusic() {
  if (musicTimer !== null || settings.muted || !settings.backgroundMusic) return;
...
export function playCardSound() {
  scheduleSlap(0, 1);
  scheduleWoodBlock(225, 0.012, 0.17);
}

export function playFlipSound() {
  scheduleNoise(0, 0.085, 0.3, 3600, 'highpass');
...
export function playCaptureSound(opponent = false) {
  ...
  speak(voiceLines.capture(opponent), 80, opponent ? 'taunt' : 'shout');
}
...
export function playBonusPeeSound(value: 2 | 3, opponent = false, chainCount = 1) {
  ...
  speak(voiceLines.bonusPee(value === 3 ? '쓰리피' : '쌍피', opponent), 190, opponent ? 'taunt' : 'shout');
}
...
export function playPeeTransferSound(value: 2 | 3, stolenByOpponent = false) {
  ...
  speak(voiceLines.peeTransfer(name, stolenByOpponent), 170, stolenByOpponent ? 'taunt' : 'shout');
}
...
export function playPpeokSound(opponent = false, count = 1) {
  ...
  speak(voiceLines.ppeok(opponent, chain), 110 + chain * 25, chain >= 2 ? 'shout' : opponent ? 'taunt' : 'low');
}
...
export function playBombSound(opponent = false) {
  ...
  speak(voiceLines.bomb(opponent), 135, opponent ? 'taunt' : 'shout');
}
...
export function playShakeSound(opponent = false) {
  ...
  speak(voiceLines.shake, 150, opponent ? 'taunt' : 'shout');
}
...
export function playSpecialMoveSound(kind: SpecialVoiceKind, opponent = false) {
  ...
  speak(voiceLines.special(kind, opponent), 115, opponent ? 'taunt' : 'shout');
}
...
export function playMissionSound(multiplier: number, opponent = false) {
  ...
  speak(voiceLines.mission(multiplier, opponent), 190, opponent ? 'taunt' : 'shout');
}
...
export function playScoreSound(label: string, score: number, opponent = false) {
  ...
  speak(voiceLines.score(label, score, opponent), 150, opponent ? 'taunt' : 'shout');
}
...
export function playGoSound(goCount: number, opponent = false) {
  if (goCount >= 5) {
    ...
    speak(voiceLines.go(goCount), 240, opponent ? 'taunt' : 'shout');
    return;
  }
  ...
  speak(voiceLines.go(goCount), 85, opponent ? 'taunt' : 'shout');
}
...
export function playStopSound(opponent = false) {
  ...
  speak(voiceLines.stop, 70, opponent ? 'taunt' : 'low');
}
...
export function playWinSound(finalScore: number, goCount: number) {
  ...
  speak(voiceLines.win(finalScore), 400, 'shout');
}
...
export function playLoseSound(bakLabels: string[] = []) {
  ...
  speak(voiceLines.lose(bakLabels), 360, 'taunt');
}
...
export function playNagariSound() {
  ...
  speak(voiceLines.nagari, 260, 'taunt');
}
...
export function playStartSound() {
  ...
  speak(voiceLines.start, 150, 'shout');
}
