...
const PLAYER_LABELS: Record<GostopPlayerId, string> = { human: '내가', computerA: '정순이가', computerB: '박영수가' };
...
export function getGostopTransitionEffect(previous: GostopRoomState | null, current: GostopRoomState): GostopTransitionEffect | null {
  ...
  if (previous.phase !== 'round-ended' && current.phase === 'round-ended') {
    if (current.roundResult === 'nagari') {
      return { kind: 'settlement', text: '나가리!', detail: `이번 판은 승부 없이 끝났습니다. 다음 판은 ${current.roundMultiplier * 2}배입니다.`, duration: 1500, player: null };
    }
    ...
    return {
      ...
      text: stopped ? '스톱!' : '승리!',
      detail: winner ? `${PLAYER_LABELS[winner]} ${current.finalScore}점으로 이겼습니다.` : `${current.finalScore}점으로 판이 끝났습니다.`,
      duration: 1600,
      player: winner
    };
  }
  ...
  for (const player of PLAYERS) {
    ...
    if (goCount > previous.players[player].goCount) {
      return { kind: 'go', text: `${goCount}고!`, detail: `${PLAYER_LABELS[player]} 승부를 계속합니다.`, duration: goCount >= 5 ? 1800 : 1150, player };
    }
  }

  if (current.turnNumber > previous.turnNumber && current.lastSpecialEvents.length) {
    const priority = ['sweep', 'self-ppeok', 'ppeok-capture', 'ppeok'];
    ...
    const detail = primary.kind === 'ppeok'
      ? `${PLAYER_LABELS[player]} 같은 월 패 세 장을 바닥에 쌌습니다.`
      : `${PLAYER_LABELS[player]} ${primary.label}에 성공했습니다.${stolenPee.length ? ` 두 상대에게서 피 ${stolenPee.length}장을 가져왔습니다.` : ''}`;
  ...
  for (const player of PLAYERS) {
    ...
    if (bonusCard) {
      ...
      return {
        ...
        text: value === 3 ? '쓰리피!' : '쌍피!',
        detail: `${PLAYER_LABELS[player]} 보너스패를 얻었습니다.`,
        duration: 1250,
        player,
        peeBurstValue: value,
  ...
  for (const player of PLAYERS) {
    ...
    return {
      kind: 'score',
      text: `${line?.label ?? '점수'}!`,
      detail: `${PLAYER_LABELS[player]} +${added}점 · 현재 ${currentScore.total}점`,
      duration: 1250,
      player
    };
  }
  ...
  for (const player of PLAYERS) {
    ...
    if (captured > 0) {
      return { kind: 'capture', text: '짝!', detail: `${PLAYER_LABELS[player]} ${captured}장을 먹었습니다.`, duration: 620, player };
    }
  }
  return null;
}
