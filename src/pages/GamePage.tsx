...
export default function GamePage() {
  ...
  useEffect(() => {
    if (!started || !game || !profile || busy || dealing || gookjinChoiceOpen || game.currentPlayer !== 'computer' || game.phase !== 'playing') return;
  ...
  }, [autoPlay, busy, dealing, game, gookjinChoiceOpen, onlineSession, preferredDifficulty, profile, started]);
  ...
  useEffect(() => {
    if (!started || !game || !profile || busy || dealing || gookjinChoiceOpen || game.phase !== 'awaiting-chongtong' || game.pendingDecision !== 'computer') return;
  ...
  useEffect(() => {
    if (!started || !game || !profile || busy || dealing || gookjinChoiceOpen || game.phase !== 'awaiting-go-stop' || game.pendingDecision !== 'computer') return;
  ...
  useEffect(() => {
    if (!autoPlay || !started || !game || !profile || busy || dealing || exitDialogOpen || floorCardChoice || bombChoice || shakeChoice || game.phase === 'round-ended') return;
  ...
  }, [autoPlay, bombChoice, busy, dealing, exitDialogOpen, floorCardChoice, game, gookjinChoiceOpen, profile, shakeChoice, started]);
  ...
  return <div className={viewportFit.className} style={viewportFit.style}>
    ...
    {!started && game.startingPlayerConfirmed === false && game.phase !== 'round-ended' ? <div className="starting-player-overlay"><section>
      ...
      <p>게임 시작을 누르면 패 치는 소리와 선언 음성이 재생됩니다. 난이도는 이번 판에 저장됩니다.</p>
