...
export default function AdminPage() {
  ...
  const resetPassword = async (user: FamilyUser) => {
    const password = prompt(`${user.displayName} 님의 새 비밀번호를 입력하세요. (4자 이상)`);
    if (!password) return;
    setError(''); setNotice('');
    ...
    <section className="admin-grid">
      <article className="panel-card">
        ...
        <form className="stack-form" onSubmit={addMember}>
          ...
          <label>첫 비밀번호 (4자 이상)<input name="password" type="password" required minLength={4} maxLength={128} /></label>
